...
import {
  ...
  type RevisionAction,
} from "@/lib/ai/parse-revision-command";
import {
  resolveActorByTelegramChatId,
...
import {
  simulateInvoice,
  commitInvoice,
} from "@/lib/workflows/telegram-invoice";
...
async function handleMessage(
  ...
  const pendingProgressReport =
    await prisma.telegramPendingProgressReport.findFirst({
  ...
  if (pendingProgressReport) {
    ...
    if (CONFIRM_WORDS.has(normalized)) {
      await prisma.telegramPendingProgressReport.deleteMany({
        where: { chatId },
      });
      try {
        ...
        await sendTelegramMessage(
          ...
          err instanceof ForbiddenError
            ? err.message
            : "Anda tidak punya izin membuat progress report.",
        );
        return;
      }
      try {
        ...
        const folder = await prisma.folder.findFirst({
          where: {
            ...
            routeKey: "PROJECT/PROGRESS_REPORT",
          },
        });
        ...
        const result = await generateProgressReportForActor(
          doc.id,
          pendingProgressReport.projectId,
          actor,
        );
        await logActivity(prisma, {
          ...
          entityId: result.progressReportId,
        ...
        const { buffer: pdfBuffer, fileName: pdfFileName } =
          await renderProgressReportPdf(result.progressReportId);
        ...
        await sendTelegramMessage(
          chatId,
          `Gagal membuat progress report: ${err instanceof Error ? err.message : "kesalahan tidak diketahui"}`,
        );
      }
      return;
    }
    ...
    await sendTelegramMessage(
      chatId,
      'Masih menunggu konfirmasi progress report sebelumnya — balas "ya" atau "batal" dulu.',
    );
    return;
  }
  ...
  if (attachment) {
    ...
    if (!isExtractableMimeType(attachment.mimeType)) {
      await sendTelegramMessage(
        chatId,
        `Tipe file "${attachment.mimeType}" tidak didukung — lampirkan PDF atau foto.`,
      );
      return;
    }
    ...
    const simulation = await simulateProgressReportFromDocument(
    ...
    if (!simulation.ok || !simulation.projectId || !simulation.projectNumber) {
      await sendTelegramMessage(
        chatId,
        simulation.error ?? "Gagal memproses permintaan progress report.",
      );
      return;
    }
    await prisma.telegramPendingProgressReport.deleteMany({
  ...
  if (costingDraft) {
    ...
    if (costingDraft.stage === "CONFIRMING" && CONFIRM_WORDS.has(normalized)) {
      try {
        ...
        await sendTelegramMessage(
          ...
          err instanceof ForbiddenError
            ? err.message
            : "Anda tidak punya izin membuat costing sheet.",
        );
        return;
      }
      try {
        ...
        await logActivity(prisma, {
          ...
          action: "CREATE",
          entityType: "COSTING",
          entityId: result.costingSheetNumber,
        ...
        await sendTelegramMessage(
          chatId,
          `Gagal menyimpan costing: ${err instanceof Error ? err.message : "kesalahan tidak diketahui"}`,
        );
      }
      return;
    }
    if (CANCEL_WORDS.has(normalized)) {
      ...
      await sendTelegramMessage(
        chatId,
        "Draft costing dibatalkan — tidak ada yang tersimpan.",
      );
      return;
    }
    ...
    if (updated.stage === "CONFIRMING") {
      ...
      await sendTelegramMessage(
        chatId,
        `Masih perlu: ${whatsMissing(updated)}.`,
      );
    }
    return;
  }
  ...
  if (pendingInvoice) {
    ...
    if (CONFIRM_WORDS.has(normalized)) {
      ...
      try {
        ...
        await sendTelegramMessage(
          ...
          err instanceof ForbiddenError
            ? err.message
            : "Anda tidak punya izin membuat invoice.",
        );
        return;
      }
      try {
        const result = await commitInvoice(
          {
            ...
            dpPercent:
              pendingInvoice.dpPercent != null
                ? Number(pendingInvoice.dpPercent)
                : null,
        ...
        await logActivity(prisma, {
          ...
          action: "CREATE",
          entityType: "INVOICE",
          entityId: result.invoiceNumber,
          description: `${result.invoiceNumber}: Dibuat via Telegram oleh ${actor.name}`,
        });
        await sendTelegramMessage(
          chatId,
          `Invoice ${result.invoiceNumber} tersimpan sebagai DRAFT. Masih perlu submit + approval Admin di app.`,
        );
      } catch (err) {
        console.error("[telegram-webhook] invoice commit failed:", err);
        await sendTelegramMessage(
          chatId,
          `Gagal menyimpan invoice: ${err instanceof Error ? err.message : "kesalahan tidak diketahui"}`,
        );
      }
      return;
    }
    ...
    await sendTelegramMessage(
      chatId,
      'Masih menunggu konfirmasi invoice sebelumnya — balas "ya" atau "batal" dulu.',
    );
    return;
  }
  ...
  if (NEW_INVOICE_TRIGGER.test(text)) {
    ...
    const simulation = await simulateInvoice(
      parsed.projectNumber,
      parsed.amount,
      parsed.dpPercent,
      parsed.dueInDays,
    ...
    if (
      ...
      !simulation.customerId ||
      simulation.amount == null ||
      !simulation.dueDate
    ) {
      await sendTelegramMessage(
        chatId,
        simulation.error ?? "Gagal memproses permintaan invoice.",
      );
      return;
    }
    await prisma.telegramPendingInvoice.deleteMany({ where: { chatId } });
  ...
  if (pending) {
    ...
    if (CONFIRM_WORDS.has(normalized)) {
      ...
      try {
        ...
        await sendTelegramMessage(
          ...
          err instanceof ForbiddenError
            ? err.message
            : "Anda tidak punya izin untuk merevisi quotation.",
        );
        return;
      }
      ...
      try {
        ...
        await sendTelegramMessage(
          chatId,
          `Gagal memproses revisi: ${err instanceof Error ? err.message : "kesalahan tidak diketahui"}`,
        );
      }
      return;
    }
    ...
    await sendTelegramMessage(
      chatId,
      'Masih menunggu konfirmasi revisi sebelumnya — balas "ya" atau "batal" dulu.',
    );
    return;
  }
  ...
  if (!parsed.quotationNumber) {
    await sendTelegramMessage(
      chatId,
      'Sebutkan nomor quotation yang mau direvisi, contoh: "Revisi Quotation 003/QUO/MKT/VI/2026, naikkan harga 10%".',
    );
    return;
  }
  ...
  const simulation = await simulateQuotationRevision(
  ...
  if (!simulation.ok || !simulation.quotationId || !simulation.costingId) {
    await sendTelegramMessage(
      chatId,
      simulation.error ?? "Gagal menghitung simulasi.",
    );
    return;
  }

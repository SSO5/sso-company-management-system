...
import {
  ...
  deleteMilestone,
  completeDeliveryMilestoneAction,
...
export function MilestonePanel({
  ...
  async function onDelete(m: Milestone) {
    if (
      !window.confirm(
        `Hapus milestone "${m.name}"? Bobotnya akan hilang dari Kurva S.`,
      )
    )
      return;
    ...
    <div className="space-y-3">
              ...
              <div>
                ...
                {m.sourcePurchaseOrder && (
                  <p className="text-[11px] text-muted-foreground">
                    Mengikuti{" "}
                    {m.dateBasis
                      ? (DATE_BASIS_LABEL[m.dateBasis] ?? "PO")
                      : "PO"}{" "}
                    {m.sourcePurchaseOrder.number}
                  </p>
                )}
                {m.description && (
                  <p className="text-xs text-muted-foreground">
              ...
              m.status !== "COMPLETED" ? (
                ...
                <>
                  <Select
                    ...
                    aria-label="Ubah status tahapan"
                    defaultValue={m.status}
                    onChange={async (e) => {
                      const res = await updateMilestoneStatus(
                        m.id,
                        projectId,
                        e.target.value as "PENDING" | "IN_PROGRESS" | "DELAYED",
                      );
                  ...
                  >
                    <Truck className="h-3.5 w-3.5" /> Tandai Selesai (upload
                    bukti)
                  </Button>
                </>
              ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen} title="New Milestone">
        <form onSubmit={onCreate} className="space-y-3">
          ...
          <div className="space-y-1">
            <Label>
              Bobot (% dari total project){" "}
              <span className="text-muted-foreground">— untuk Kurva S</span>
            </Label>
            <Input
              name="weightPercent"
              type="number"
      ...
      >
        {editing && (
          <form key={editing.id} onSubmit={onEdit} className="space-y-3">
            <div className="space-y-1">
              <Label>Jenis Pekerjaan / Nama</Label>
            ...
            <div className="space-y-1">
              ...
              {editing.sourcePurchaseOrder ? (
                <>
                  <Input
                    type="date"
                    defaultValue={toDateInputValue(editing.dueDate)}
                    disabled
                  />
                  <p className="text-[11px] text-muted-foreground">
                    Tanggal ini mengikuti{" "}
                    {editing.dateBasis
                      ? (DATE_BASIS_LABEL[editing.dateBasis] ?? "PO")
                      : "PO"}{" "}
                    {editing.sourcePurchaseOrder.number} — ubah dari tombol Edit
                    PO di tab Documents, bukan di sini, supaya tidak ada dua
                    sumber tanggal yang berbeda.
                  </p>
                </>
              ) : (
                <Input
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                max={100}
                step="0.01"
                defaultValue={Number(editing.weightPercent)}
              />
            </div>
            <div className="space-y-1">
              <Label>Keterangan (opsional)</Label>
        ...
        {completing && (
          <form
            onSubmit={async (e) => {
              ...
              const fd = new FormData(e.currentTarget);
              setPending(true);
              const res = await completeDeliveryMilestoneAction(
                completing.id,
                projectId,
                fd,
              );
              ...
              if (res.ok) {
                ...
                toast({
                  title: "Tidak bisa menandai selesai",
                  description: res.error,
                  variant: "destructive",
                });
            }}
          ...
          >
            <div className="space-y-1">
              <Label>
                Bukti Pengiriman (surat jalan/BAST){" "}

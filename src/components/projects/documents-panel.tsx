...
const CUSTOMER_PO_VARIANT: Record<
  string,
...
const VENDOR_PO_VARIANT: Record<
  string,
  "default" | "secondary" | "success" | "warning" | "destructive" | "outline"
> = {
  DRAFT: "secondary",
...
export function DocumentsPanel({
    ...
    .reduce(
      (s, i) =>
        ...
        invoiceDueAmount({
          ...
          dpPercent: i.dpPercent as number | null,
        }),
      0,
    );
  ...
  return (
    <div className="space-y-4">
      {activePOs.length > 0 && (
        <Section
          title="Timeline Penagihan"
          hint="PO → DP → tiap termin → Pelunasan, ditarik langsung dari data Keuangan"
        >
          <div className="grid grid-cols-3 gap-3 text-sm">
            ...
            <div>
              ...
              <p
                className={`font-semibold ${remainingToBill > 0 ? "text-warning" : "text-success"}`}
              >
            ...
            <div className="relative mt-4 space-y-4 border-l-2 border-border py-1 pl-5">
              {billingTimeline.map((step) => (
                <div key={step.key} className="relative">
                  <span
                    className={cn(
                      "absolute -left-[25px] top-0.5 h-3 w-3 rounded-full border-2 border-card",
                      TIMELINE_DOT[step.status],
                    )}
                  />
          ...
          {termsToShow.length === 0 && remainingToBill > 0 && (
            <p className="mt-3 border-t border-border pt-3 text-xs text-muted-foreground">
              Term of Payment belum tercatat untuk PO ini — tidak bisa
              menentukan tahap penagihan berikutnya secara otomatis. Lengkapi
              lewat &quot;Upload PO (AI-assisted)&quot; atau edit manual.
            </p>
          )}
        </Section>
      )}
          ...
          <div className="mt-3 flex flex-wrap gap-3 border-t border-border pt-3 text-sm">
            {opportunity && (
              <Link
                ...
                className="flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 hover:bg-accent"
              >
            ...
            {quotation && (
              ...
              >
                <FileText className="h-3.5 w-3.5 text-muted-foreground" />{" "}
                Quotation{" "}
                {formatRevisedNumber(quotation.number, quotation.revision)}
        ...
        action={
          purchaseOrderFolderId && (
            <PoExtractUploadDialog
              folderId={purchaseOrderFolderId}
              projectId={projectId}
      ...
      >
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className="font-medium">
                    {formatCurrency(Number(po.poValue))}
                  </span>
                  <Badge variant={CUSTOMER_PO_VARIANT[po.status]}>
                    {po.status}
                  </Badge>
                </div>
              </div>
        ...
        <p className="mt-3 border-t border-border pt-2 text-[11px] text-muted-foreground">
          Data PO ini hanya bisa dikoreksi oleh Sales, lewat{" "}
          <Link
            ...
            {vendorPurchaseOrders.map((po) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Badge variant={VENDOR_PO_VARIANT[po.status]}>
                    {po.status}
                  </Badge>
                  <a
                    href={`/api/procurement/vendor-po/${po.id}/pdf`}
                    target="_blank"
                    rel="noreferrer"
                  ...
                  <span className="font-medium">
                    {formatCurrency(
                      invoiceDueAmount({
                        ...
                        dpPercent: inv.dpPercent as number | null,
                      }),
                    )}
                  </span>

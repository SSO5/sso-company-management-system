...
export async function listUploadCandidateProjects(): Promise<
  CandidateProject[]
> {
...
> {
  return runAction(async () => {
    ...
    if (file.size > 4 * 1024 * 1024)
      throw new Error(
        "Batas unggah AI 4 MB. Kompres dokumen sebelum mengunggah.",
      );
    ...
    if (!isExtractableMimeType(mimeType)) {
      return {
        ...
        extracted: null,
        extractionError:
          "Tipe file ini tidak didukung untuk klasifikasi otomatis (hanya PDF/gambar) — pilih project dan folder secara manual.",
        projects,
      };
    }

    try {
      const extracted = await classifyDocument(
        buffer,
        mimeType,
        file.name,
        projects,
      ...
      return {
        ...
        extractionError:
          e instanceof Error
            ? e.message
            : "Klasifikasi AI gagal — pilih project dan folder secara manual.",
        projects,
      };
    }
  });
...
export async function confirmSmartUpload(
  ...
  ActionResult<{ id: string; folderPath: string; projectId: string }>
> {
  return runAction(async () => {
    const actor = await requireUserOrThrow();
    requirePermission(actor.role, "documents", "create");
    ...
    const file = formData.get("file") as File | null;
    if (!file || file.size === 0)
      throw new Error("File hilang — coba upload ulang.");
    if (file.size > 4 * 1024 * 1024) throw new Error("Batas unggah AI 4 MB.");
    ...
    const project = await prisma.project.findFirst({
      where: { number: params.projectNumber, deletedAt: null },
    });
    if (!project)
      throw new Error(`Project "${params.projectNumber}" tidak ditemukan.`);

    const folder = await prisma.folder.findFirst({
      where: { projectId: project.id, routeKey: params.routeKey },
    });
    if (!folder)
      throw new Error(
        "Folder tujuan tidak ditemukan untuk project ini — coba pilih folder lain.",
      );
    ...
    const doc = await uploadDocument(
      {
        ...
        mimeType,
        folderId: folder.id,
        description: `Unggah terarah; sumber asli: ${file.name}; tujuan ditinjau pengguna.`,

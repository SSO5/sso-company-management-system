...
export async function getSalesReport() {
  ...
  const revenueBySalesPic = Array.from(bySalesPic.entries()).map(
  ...
  return {
    totalOpportunities,
    totalQuotationValue: quotations.reduce(
      (s, q) => s + Number(q.grandTotal),
      0,
    ),
    wonValue,
    lostValue,
...
export async function getFinanceReport() {
  ...
  const [allInvoices, expenses, companyExpenses] = await Promise.all([
  ...
  const invoices = allInvoices.filter((i) => issuedInvoice(i.status));
  ...
  const revenue = totals.billed; // Legacy field name: billed amount, not recognized revenue.
  const paid = totals.cash,
    withholdingTax = totals.withholding,
    outstanding = totals.outstanding;
  const overdue = invoices
    .filter((i) => i.dueDate < new Date())
    .reduce((s, i) => s + invoiceOutstanding(i), 0);
  const projectExpenses = expenses.reduce((s, e) => s + Number(e.total), 0);
  const operatingExpenses = companyExpenses.reduce(
    (s, e) => s + Number(e.total),
    0,
  );
  const totalExpenses = projectExpenses + operatingExpenses;
  const grossProfit = null; // Invoices and costs alone do not establish an income statement.
  const profitStatus =
    "Laba aktual belum tersedia: membutuhkan pengakuan pendapatan, kelengkapan biaya, dan rekonsiliasi.";
  ...
  for (const e of [...expenses, ...companyExpenses]) {
...
export async function getProjectReport() {
  ...
  const activeRows = projects
    ...
    .map((p) => {
      ...
      return {
        ...
        budget: Number(p.budget),
        cost,
        scheduleGap: Math.max(
          0,
          sCurve.asOfToday.planned - sCurve.asOfToday.actual,
        ),
        signals,
      };
    });

  const riskyProjects = activeRows
    .filter((r) => r.signals.length > 0)
    .sort((a, b) => b.signals.length - a.signals.length);
  const avgScheduleDeviation = activeRows.length
    ? Math.round(
        activeRows.reduce((s, r) => s + r.scheduleGap, 0) / activeRows.length,
      )
    : 0;
  ...
  return {
    ...
    statusBreakdown: Array.from(byStatus.entries()).map(([name, value]) => ({
      name,
      value,
    })),
    ...
    riskyProjects: riskyProjects.map((r) => ({
      number: r.number,
...
export async function getProfitabilityReport() {
  ...
  return projects.map((p) => {
    ...
    const revenue = estimate ? Number(estimate.netSales) : null;
    const cost = estimate ? Number(estimate.totalForecastCost) : null;
    const forecast = estimate
      ? forecastMargin(
          Number(estimate.netSales),
          Number(estimate.totalForecastCost),
        )
      : null;
    return {
      id: p.id,
      ...
      revenue,
      cost,
      grossProfit: forecast?.margin ?? null,
      grossMargin: forecast?.percent ?? null,
...
export async function getExecutiveReport() {
  const [sales, finance, project] = await Promise.all([
    getSalesReport(),
    getFinanceReport(),
    getProjectReport(),
  ]);

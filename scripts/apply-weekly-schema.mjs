...
if (process.env.VERCEL !== "1") {
  ...
  for (const migration of ["weekly", "finance"]) {
    const sql = readFileSync(
      new URL(`../prisma/deploy/20260914-${migration}.sql`, import.meta.url),
      "utf8",
    ).replace(/\r\n/g, "\n");
    const checksum = createHash("sha256").update(sql).digest("hex");
    const name = `20260914-${migration}-v1`;
    try {
      ...
      // Connection is closed after both additive migrations.
    }
